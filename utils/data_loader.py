...
def read_cf(file_name):
    ...
    for l in lines:
        ...
        u_id, pos_ids = inters[0], inters[1:] # user = line[0] ... items = line[1:] ... ## positive interaction
        pos_ids = list(set(pos_ids)) # 移除重複的item
        for i_id in pos_ids:
            inter_mat.append([u_id, i_id]) # adjacent list user-item pair放到list中
...
def remap_item(train_data, test_data):
    ...
    #
    n_users = max(max(train_data[:, 0]), max(test_data[:, 0])) + 1 # why pick maxValus between train and test?
...
def read_triplets(file_name):
    ...
    if args.inverse_r: # default is true
    ...
    n_nodes = n_entities + n_users # entities+ item + user
...
def build_graph(train_data, triplets):
    ...
    for u_id, i_id in tqdm(train_data, ascii=True):
        rd[0].append([u_id, i_id]) # determine user-item interaction

    print("\nBegin to load knowledge graph triples ...")
    for h_id, r_id, t_id in tqdm(triplets, ascii=True):
        ckg_graph.add_edge(h_id, t_id, key=r_id) # create bi direction edge
...
def load_data(model_args):
    ...
    # --- Sub-sample KG triples according to kg_ratio --- #
    if args.kg_ratio < 1.0:
        # Set seed for reproducibility (optional)
        np.random.seed(getattr(args, 'seed', 42))
        n_keep = int(len(triplets) * args.kg_ratio)
        idx = np.random.choice(len(triplets), size=n_keep, replace=False)
        triplets = triplets[idx]

    print('building the graph ...')
    graph, relation_dict, kg_dict = build_graph(train_cf, triplets) # graph

    print('building the adj mat ...')
    # relation_dict 代表relation_dict[0] -> list[list[user_a, item_b] ...list[user_i, item_j] .....]
