...
class Evaluator:
    ...
    def test(self, model, user_dict, n_params):
        ...
        test_user_set = user_dict['test_user_set'] ## testing set
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            if self.batch_test_flag:
                ...
                for i_batch_id in range(n_item_batchs):
                    ...
                    # item_indices 就是 [i_start, i_start+1, ..., i_end-1]
                    item_indices = list(range(i_start, i_end))

                    i_rate_batch = model.scores(
                        user_list_batch,  # list of UIDs
                        item_indices,  # list of item IDs
                        u_g_embeddings,  # [B, D]
                        i_g_embddings  # [I_batch, D]
                    ).detach().cpu().numpy()
